...
var painVal = function() {
  var l = $$("input[name='pain-box']:checked").length;
  if ($$("input[name='pain-box']:checked").val() === "x") {
    return 2;
  } else if (l === 2) {
    return 1;
  } else if (l > 2) {
...
function checkForm() {
  if (
    ...
    painVal() > 0 ||
    $$("input[name='ecg']:checked").val() ||
    $$("input[name='risk-box']:checked").val() ||
  ...
  var heartscore = 0;
  heartscore = tniScore() +
    parseInt($$("input[name='age']:checked").val()) +
    painVal() +
    parseInt($$("input[name='ecg']:checked").val()) +
    riskVal();
  if (heartscore < 4) {
    //LOW RISK
    ...
    if (tniVal() < 5) {
      ...
      $$("p.second").html(
        "Tag 1h-prov, om &Delta; < 6 AKS ej sannolikt, om &Delta; > 6 handlägg som AKS"
      );
      //New test 1h, Delta below 6 not probable, over 6 treat as AKS
    } else if (tniVal() >= tniLow()) {
      $$(".bottom-toolbar").css("background", "#f57c00");
      $$("p.second").html(
        "Stämmer symptomen? Dynamik? Diskutera med medicinjour/kardiologjour? - AKS?"
      );
    ...
    setTimeout(
      function() {
        $$(".icon-keypad-delete")
          ...
          .html("backspace")
          .removeClass("icon-keypad-delete");
      },
      10
    );
  }
...
if (
  parseInt(
    new Date().getTime() - parseInt($$("input[name='storetime']").val())
  ) > 300000
) {
  $$('input[type="numpad"]')[0].f7Keypad.setValue("");
